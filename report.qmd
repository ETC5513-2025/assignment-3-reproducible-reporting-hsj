...
author: 
  - name: Sia Chawla Chawla
    contribution: Exclusive Summary, Discussion
  - name: Tran Dang Huy
    contribution: Result Interpretations, Reference
  - name: Jiaqi Xu
    contribution: Methodology, Result Graphics 
...
After fitting the random forest model, we use permutation importance to rank all input variables (@fig-rfimportance). This method measures how much the model’s accuracy drops when the values of one variable are randomly shuffled. A larger drop means the variable was more important for the model’s predictions. This allows us to assess which factors the model relies on most when estimating job offers.
